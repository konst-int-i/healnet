...
class HealNet(nn.Module):
    def __init__(
        ...
        *,
        ...
        super().__init__()
        assert len(num_tokens) == len(spatial_axes), 'input channels and input axis must be of the same length'
        assert len(spatial_axes) == modalities, 'input axis must be of the same length as the number of modalities'

        self.input_axes = spatial_axes
        self.input_channels=num_tokens
        ...
        for f_channels, i_channels in zip(fourier_channels, num_tokens):
