...
class MultiModNModule(nn.Module):
    def __init__(self,
        ...
        self.state = TrainableInitState(device=self.device)
        ...
        self.model = nn.ModuleList(encoders).to(self.device)
        self.decoder = nn.ModuleList(decoders).to(self.device)
    ...
    def forward(self, x: List[torch.Tensor], target: torch.Tensor) -> torch.Tensor:
        ...
        for encoder, mod in zip(self.encoders, x):
            ...
            self.state = encoder(self.state, mod)
